...
namespace SpaceFox
{
    ...
    public class SphereChunk : DisposableMonoBehaviour
    {
        ...
        private static Vector3 GetRelativeHeight(Vector3 direction)
        {
            ...
            var debugCellsCount = 4f;
            var uv = normailised * debugCellsCount;

            var gridId = new Vector3(Mathf.Floor(uv.x), Mathf.Floor(uv.y), Mathf.Floor(uv.z));
            var gridUV = uv - gridId;
            //gridUV = quintic(gridUV);

            var p00 = gridId + Vector3.zero;
            var p01 = gridId + Vector3.right;
            var p10 = gridId + Vector3.up;
            var p11 = gridId + Vector3.one;
            ...
            var distP00 = gridUV - Vector3.zero;
            var distP01 = gridUV - Vector3.right;
            var distP10 = gridUV - Vector3.up;
            var distP11 = gridUV - Vector3.one;
            ...
            return normailised * (dotP * 0.1f + 1f);
        }
